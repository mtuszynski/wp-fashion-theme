...
if ( is_home() ) {
	$data['post']       = new TimberPost();
	$data['posts']      = new Timber\PostQuery();
	$data['pagination'] = $data['posts']->pagination();
	...
	$data['category'] = reset( $categories );
	$template         = array(
	...
	$data['queried_object'] = get_queried_object();
	$template               = array(
			'archive/archive-' . $post->post_type . '.twig',
		// 'archive/archive-' . get_queried_object()->slug . '.twig',
